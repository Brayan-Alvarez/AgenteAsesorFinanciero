...
POST   /api/transactions/db/migrate-category → bulk reassign transactions to another category
...
### Phase 9 — Suscripciones recurrentes ✅ COMPLETE
*Suscripciones mensuales (Netflix, Spotify, etc.) se registran una vez y generan transacciones automáticas cada mes.*

**SQL migration requerida en Supabase (ejecutar una vez):**
```sql
-- Tabla de suscripciones recurrentes
CREATE TABLE subscriptions (
  ...
  end_date       date,   -- set on cancellation (soft delete)
  notes          text,
  created_at     timestamptz DEFAULT now()
);

-- Columna para vincular transacciones auto-creadas a su suscripción
ALTER TABLE transactions ADD COLUMN IF NOT EXISTS subscription_id uuid REFERENCES subscriptions(id);
```

**Arquitectura:**
- `subscriptions` tabla con soft delete (`end_date` + `is_active = false` al cancelar)
- `transactions.subscription_id` → FK a la suscripción que generó la transacción
- `process_pending_subscriptions(year, month)` — idempotente, crea transacciones faltantes
- Auto-procesamiento en AppContext.useEffect: al cargar la app se procesa el mes actual
- Si `billing_day > último día del mes`, se clampea al último día real del mes
- Suscripciones canceladas a mitad de mes siguen siendo cobradas ese mes

**Cambios implementados:**
- [x] SQL migration: tabla `subscriptions` + columna `transactions.subscription_id`
- [x] `api/models.py` — `SubscriptionCreate`, `SubscriptionUpdate`
- [x] `db/queries.py` — `get_subscriptions`, `create_subscription`, `update_subscription`, `cancel_subscription`, `process_pending_subscriptions`
- [x] NEW `api/routes/subscriptions.py` — CRUD + POST /process
- [x] `api/main.py` — registra `subscriptions_router`
- [x] `frontend/src/api/client.js` — `getSubscriptions`, `createSubscription`, `updateSubscription`, `cancelSubscription`, `processSubscriptions`
- [x] `frontend/src/context/AppContext.jsx` — carga suscripciones; auto-procesa mes actual; `subscriptionId` en `mapTxn`; patchers: `addSubscriptionLocal`, `removeSubscriptionLocal`, `reloadSubscriptions`
- [x] `frontend/src/pages/Budget.jsx` — pestaña "Suscripciones": cards, form, KPIs, anotación en filas de presupuesto
- [x] `frontend/src/pages/Transactions.jsx` — badge "🔄 sub" en transacciones automáticas
